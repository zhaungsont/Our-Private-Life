...
def booking():
    ...
    age = flask_request.form.get('age')
    ...
    url=f"https://www.booking.com/searchresults.zh-tw.html?label=booking-name-yefrPbbyS*FIINHgyCnmNgS411022708271%3Apl%3Ata%3Ap1%3Ap22%2C563%2C000%3Aac%3Aap%3Aneg%3Afi%3Atikwd-65526620%3Alp9040379%3Ali%3Adec%3Adm%3Appccp%3DUmFuZG9tSVYkc2RlIyh9YfqnDqqG8nt1O4nYvDr1lms&sid=a54a62923cbb9d7afe34b34af03d7ecd&aid=376396&ss=%E5%8F%B0%E5%8C%97&ssne=%E5%8F%B0%E5%8C%97&ssne_untouched=%E5%8F%B0%E5%8C%97&lang=zh-tw&sb=1&src_elem=sb&dest_id=-2637882&dest_type=city&checkin={year1}-{month1}-{date1}&checkout={year2}-{month2}-{date2}&group_adults={ad}&no_rooms={room}&group_children={ch}&age={age}&sb_travel_purpose=leisure&order=class"
    print("***********")
    print(url)
    print("***********")
    ...
    #抓取飯店連結
    sel = root.find_all("a",class_=order_url_class)
    for s in sel:
        bkurl=str(s).split("href=\"")[1].split("\" rel")[0]
        order_url.append(bkurl)
    #抓取價錢資料
    money_list=[]
    ...
    return render_template('booking.html', data=results, year1=year1, month1=month1, date1=date1, year2=year2, month2=month2, date2=date2, ad=ad, ch=ch, age=age, room=room)
